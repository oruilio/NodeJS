...
.get(cors.cors, (req,res,next) => {
    Dishes.find({})                //every time retrive dish information
...
.post(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {   //[authenticate.verifyUser] have loaded the user information into the req.user (contain id)
    Dishes.findById(req.params.dishId)
    .then((dish) => {
        if (dish != null) {
            ...
            .then((dish) => {
                Dishes.findById(dish._id)
                    .populate('comments.author')
                    .then((dish) => {     //push the result back to the client should be done after we retrieve the information of the author
                        res.statusCode = 200;
                        res.setHeader('Content-Type', 'application/json');
                        res.json(dish);     
                    })            
